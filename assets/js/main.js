...
//AVOIDING GLOBAL VARIABLES WITH {}:
{
  //HELPER FN:
  ...
  let text = document.createTextNode('Super Quiz!');

  h1.appendChild(text);
  $('#content').appendChild(h1);

  //MAKE THE COPY OF OBJ:
...
`;
  ...
  (function renderHtml(items) {
    items.forEach((el, i) => createHtmlTag(el, i));
  })(deepCopy(data));

  // ANSWER TRUE OR FALSE:
  function addAnimation(e, className, animation) {
    let { classList, style } = e.target;
  ...
  $on($$('span'), 'click', (e) => {
    let { innerHTML, className } = e.target;
    let answerIdx = className.split(' ')[0];

    innerHTML == String(data[answerIdx].answer)
      ? addAnimation(e, 'green', 'true 0.4s linear')
      : addAnimation(e, 'red', 'false 0.4s linear');
  });
}
